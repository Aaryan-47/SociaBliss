...
function Followers({user,loggedUserFollowStats,setUserFollowStats,profileUserId})
{
    ...
    useEffect(()=>{
        ...
        const getFollowers=async()=>{
            setLoading(true)
            try{
               const res=await axios.get(`${baseUrl}/api/profile/followers/${profileUserId}`,{headers:{Authorization:cookie.get("token")}})
  ...
  return(
    <>
      {loading?(<Spinner/>):followers.length>0? followers.map(profileFollower=>{
                         ...
                         {profileFollower.user._id!==user._id && (
                            <Button color={isFollowing?"instagram":"twitter"} 
                             ...
                             onClick={async()=>{
                                ...
                                isFollowing ? await unfollowUser(profileFollower.user._id,setUserFollowStats) :await followUser(profileFollower.user._id,setUserFollowStats)
